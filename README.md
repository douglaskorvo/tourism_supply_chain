...
| `validation/` | Companion notebooks defining the held-out validation procedure: sampling and blind workbooks (`validation_prep.ipynb`), agreement statistics and the query-stratum face-validity audit (`validation_scoring.ipynb`), and gold labels with a provider-grouped 60/30/10 split (`validation_split.ipynb`). Code only — the coding records they read are not deposited |
...
As of v1.2.0 the repository deposits the validation *procedure* — see `validation/` — but neither its inputs nor its results. Those notebooks define how the held-out double-coded sample is drawn, which agreement statistics are computed on returned codings, how the provider query-stratum audit is scored, and how the gold-labelled train/validation/test split is built without provider leakage. They read workbooks that are not distributed here and produce nothing without them. Individual human-coding records, provider-audit records and the resulting validation metrics remain outside the scope of this public release, and no such metric is reported in this repository.

**Provider strata are retrieval artefacts.** The `mass_market` variable records which retrieval query returned a provider; it is not a verified organizational characteristic of the firm. The validation materials therefore name it a *query stratum* — mass-market versus creative-tourism — and read results as patterns associated with the retrieval strategy rather than as properties of the providers. An earlier audit that asked an independent assessor to recover an organizational typology from this label returned chance-level agreement (Cohen's κ ≈ −0.01, n = 399); that negative result is documented in `validation/` as a limit of the proxy, and the audit was redesigned to test what the label can support — whether each provider is a plausible hit for the query stratum that retrieved it. Column names in `tssc_analysis.ipynb` and `reference_outputs/` are unchanged for compatibility.
